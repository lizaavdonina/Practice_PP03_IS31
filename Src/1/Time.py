...
class ImageUtilityApp:
    ...
    def load_image(self):
        ...
        print(f"Время загрузки изображения: {elapsed:.6f} секунд")
        print(f"Размер объекта изображения: {sys.getsizeof(self.image)} байт")
    ...
    def resize_image(self):
        ...
        if new_width is not None and new_height is not None:
            ...
            elapsed = timeit.default_timer() - start_time
            print(f"Время изменения размера изображения: {elapsed:.6f} секунд")
            print(f"Размер объекта измененного изображения: {sys.getsizeof(resized_image)} байт")
